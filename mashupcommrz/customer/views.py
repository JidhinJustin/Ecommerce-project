...
from customer.forms import RegistrationForm, CustomerCheckoutForm
from apppannel.forms import LoginForm
...
def registercustomer(request):
    if request.method == 'POST':
        ...
        if registerform.is_valid():
            ...
            if  User.objects.filter(username=username).exists():
                ...
                return render(request,'/customer/partials/customers/registercustomer.html',context)
            else:	
    ...
    return render(request,'customer/partials/customers/registercustomer.html',{'registerform':registerform})
            ...
            if login_form.is_valid():
                ...
                if user is not None:
                    if user.is_active:
                        ...
                        login_form = LoginForm(request.POST)
                        return render(request, "customer/partials/customers/logincustomer.html",{"form":login_form})
                else:
                    login_form = LoginForm(request.POST)
                    return render(request, "customer/partials/customers/logincustomer.html",{"form":login_form})
            else:
...
def homepage(request):  
    ...
    return render(request,'customer/partials/customers/products.html',{'products':products,'usercart':usercart})
...
def viewcustomercart(request):
    ...
    return render(request,'customer/partials/customers/customercart.html',{'usercart':usercart,
...
def checkoutcustomer(request):
    if request.method == 'POST':
        ...
        client = razorpay.Client(auth=("rzp_test_aKWjnDS2VeSfrl", "4W37XrSU6057JMLAz8MOJtU5"))
        DATA = {
        ...
        context = {'order_id' : order_details.get('id'),
                    ...
                    'phonenum' : phone,
                    'rzpkey' : 'rzp_test_aKWjnDS2VeSfrl'
                    }
        return render(request,'customer/partials/customers/checkout.html',context)
    else:
      return HttpResponseRedirect(reverse('products'))  
